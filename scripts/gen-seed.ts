...
insert into projects (id, account_id, name, target, stage, client_name, client_domain, calendar_link, relevant_links) values
  ('${AURORA_ID}', '${ACCOUNT_ID}', 'Aurora Press', 'ollie', 'approval', 'Aurora Coffee Roasters', 'auroracoffee.com', 'https://cal.com/series5/aurora', '${esc([
  ...
  ('${NORTHWIND_ID}', '${ACCOUNT_ID}', 'Northwind Traders', 'ollie', 'onboarding', null, null, null, '[]'),
  ('${MORI_ID}', '${ACCOUNT_ID}', 'Studio Mori', 'ollie', 'design', null, null, null, '[]'),
  ('${HARBOR_ID}', '${ACCOUNT_ID}', 'Harbor Health', 'ollie', 'content', null, null, null, '[]')
on conflict (id) do nothing;
